...
电脑端打开“项目管理”后，可以为每个已允许的项目选择权限策略：

- `需要时询问`：默认策略，Codex 请求执行受限操作时由手机确认
- `失败时询问`：先尝试执行，失败时再请求确认
- `自动执行`：不弹出审批，适合完全信任的项目；请谨慎使用

权限策略只允许在电脑端修改，保存在 `.codex-pocket/project-settings.json`，手机端会显示当前策略但不能绕过电脑端设置。
